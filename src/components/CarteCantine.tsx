...
export const CARTE_CANTINE_STYLES = `
  .carte-cantine {
  ...
  /* === RECTO === */
  .carte-recto {
    background: linear-gradient(160deg, #34d399 0%, #059669 18%, #047857 50%, #065f46 100%);
  ...
  .carte-recto .alert-bar {
    ...
    text-align: center; letter-spacing: 0.15mm; text-transform: uppercase;
    display: flex; align-items: center; justify-content: center; gap: 1.5mm;
    border-top: 0.3mm solid #fecaca;
  }
...
export default function CarteCantine({
  nom, prenom, matricule, classe, photo_url, allergies = [], regime,
  telephone_pere, telephone_mere, qrValue,
  schoolName = 'École Internationale Les Enfants du Futur',
  schoolLogo, hidePrintButton = false,
}: CarteCantineProps) {
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  @media print { body { background:white;min-height:auto;gap:2mm; } }
  ...
  return (
    <div className="space-y-4">
      <div ref={printRef}>
        <style>{CARTE_CANTINE_STYLES}</style>
        <CarteRectoVerso
          nom={nom} prenom={prenom} matricule={matricule} classe={classe}
          photo_url={photo_url} allergies={allergies} regime={regime}
          telephone_pere={telephone_pere} telephone_mere={telephone_mere}
          qrValue={qrValue} schoolName={schoolName} schoolLogo={schoolLogo}
          hasAlerts={!!hasAlerts}
        />
      </div>
      {!hidePrintButton && (
...
/** Recto + Verso exported for reuse in A4 planche */
export function CarteRectoVerso({
  nom, prenom, matricule, classe, photo_url, allergies = [], regime,
  telephone_pere, telephone_mere, qrValue,
  schoolName = 'École Internationale Les Enfants du Futur',
  schoolLogo, hasAlerts,
}: {
  nom: string; prenom: string; matricule: string; classe: string;
  photo_url?: string | null; allergies?: string[]; regime?: string | null;
  telephone_pere?: string | null; telephone_mere?: string | null;
