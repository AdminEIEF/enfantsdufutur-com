...
export default function ParentEnfantPointage({ pointages }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Today status */}
      {todayEntry ? (
        <Card className={`border-2 ${todayEntry.heure_depart ? 'border-orange-400' : 'border-emerald-400'}`}>
          <CardContent className="pt-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {!todayEntry.heure_depart && todayEntry.heure_arrivee && (
            ...
            {sorted.map(p => (
              <div key={p.id} className="flex items-center justify-between text-sm border rounded px-3 py-2">
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  {p.heure_arrivee && (
                    <span className="text-emerald-600">↓ {format(new Date(p.heure_arrivee), 'HH:mm')}</span>
                  )}
                  {p.heure_depart && (
